...
2.  **安装依赖**:
    ...
    ```
    这将安装 `fastapi`, `uvicorn`, 和 `py7zr`。
    **注意**: RARファイルの解凍には、別途 `7-Zip` (7z.exe) がシステムにインストールされている必要があります。

3.  **7-Zip のインストール (RAR解凍に必要)**:
    RARファイルを解凍するには、`7-Zip` コマンドラインユーティリティ (`7z.exe` または `7z`) がシステムにインストールされ、実行パスに含まれている必要があります。
    `7-Zip` は公式サイト (https://www.7-zip.org/) からダウンロードできます。
    インストール後、`7z` または `7z.exe` がコマンドプロンプト/ターミナルから実行できることを確認してください。
    あるいは、`config.py` ファイル内の `SEVENZ_EXE_PATH` に `7z.exe` のフルパスを設定することも可能です。
...
3.  根据需要选择以下选项:
    ...
    *   **Extract Flat (Ignore Folders, Extract Files Only)**: 是否将所有文件（无论原先在哪个子文件夹）直接提取到目标路径，忽略所有内部文件夹结构。(默认为不勾选)
        *   **同名文件处理**:
            *   ZIP, TAR, 7Z (py7zr使用時): 展开时若目标位置已存在同名文件，会比较文件大小。大小一致则跳过该文件；大小不一致则在文件名后附加数字（如 `file_1.txt`）以避免覆盖。
            *   RAR (7z.exe使用時): 展开时若目标位置已存在同名文件，`7z.exe` 的默认行为是直接覆盖。大小比较和重命名逻辑は適用されません。
...
| :------ | :-------------------- | :---------------------------- | :------- |
| ZIP     | .zip                  | 内置                          | 检测     |
| RAR     | .rar                  | `7z.exe` (7-Zip)              | 限定的检测* |
| 7-Zip   | .7z                   | `py7zr` (または `7z.exe` 将来的に) | 检测     |
| TAR     | .tar                  | 内置                          | -        |
...
5.  **密码保护**: 工具无法处理受密码保護的压缩文件；它们会被跳过。
6.  **嵌套深度**: 理论上支持无限层级嵌套，但受系统资源和 `MAX_PROCESSING_ROUNDS` (在 `config.py` 中定义) 的限制。
7.  **Windowsでの長いパス名**: Windows環境では、ファイルパスの長さにOSレベルの制限（約260文字）があります。本ツールではこの制限を超える可能性のある長いパス名を扱うために、内部的に対策（`\\?\`プレフィックスの使用）を施していますが、非常に深い階層構造や極端に長いファイル名を持つアーカイブの場合、予期せぬ問題が発生する可能性が完全には排除できません。もし解凍時に `No such file or directory` のようなエラーが長いパスで発生する場合、OSのロングパスサポートを有効にする（Windows 10 Creators Update以降で設定可能）、または解凍先のパスを浅い階層に変更するなどの対策をお試しください。

## 🐛 故障排除

### 常见问题
1.  **RAR文件无法解压**:
    *   システムに `7-Zip` がインストールされ、`7z.exe` (または `7z`) がコマンドラインから実行可能であることを確認してください。
    *   `7-Zip` がインストールされていてもパスが通っていない場合、`config.py` の `SEVENZ_EXE_PATH` に `7z.exe` のフルパスを設定してみてください (例: `"C:/Program Files/7-Zip/7z.exe"`)。
    *   `7z.exe` がパスワードを要求している場合（ログで確認）、そのファイルはパスワードで保護されています。このツールはパスワード入力に対応していません。
2.  **7Zファイル无法解压 (py7zr を使用する場合)**:
    *   `requirements.txt` により `py7zr` がインストールされていることを確認してください。
3.  **编码错误**: 工具会自动尝试多种常见编码。如果仍有问题，可能遇到了非常罕见的编码或损坏的文件名。
4.  **权限不足**: 检查程序对目标目录和日志目录的读写权限。
4.  **UI没有反应/按钮点击无效**:
